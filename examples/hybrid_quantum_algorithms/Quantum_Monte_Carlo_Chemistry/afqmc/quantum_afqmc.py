...
def local_energy_quantum(
    ...
    Args:
        ...
        one_body (ndarray): (corrected) one-body term in the second quantized hamiltonian written in chemist's notation.
        ...
        dev (device): qml.device('lightning.qubit', wires=wires) for simulator;
...
def U_circuit(U: np.ndarray):
    ...
    Args:
        U (np.ndarray): Unitary transformation.
    """
...
def pauli_estimate(
    ...
    Args:
        Q (ndarray): np.ndarray; matrix representation of the walker state, not necessarily orthonormalized.
...
def q_propogate_walker(
    ...
    Returns:
        ndarray: Walker for the next time step
    """
...
def one_body_expectation(
    ...
    Args:
        ...
        one_bodies (List[ndarray]): list of one_body operators whose expectation value is to be computed;
        ovlp (float): amplitude between walker and the quantum trial state
        V_T (Callable): quantum trial state
        dev (device): qml.device('lightning.qubit', wires=wires) for simulator;
            i.e. qml.device('braket.aws.qubit', device_arn=device_arn, wires=wires, shots=shots) for real device;
    Returns:
