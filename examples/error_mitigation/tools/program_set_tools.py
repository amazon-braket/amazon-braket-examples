...
def _convert_circuit(
    circuit: Circuit, parameter: dict, pstr: str, conversion=None, verbatim: bool = False
) -> Circuit:
    """ """
...
def _zip_to_pset(
    ...
    circuits = [
        _convert_circuit(c, p, o, conversion=conversion, verbatim=verbatim)
        for c, p, o in zip(*zip(*obj_list))
    ]
    return ProgramSet(circuits, shots_per_executable=shots_per_executable)
...
def distribute_to_program_sets(
    ...
    if n_circ * n_para * n_obs > PROGRAM_SET_LIMIT:
        print(
            f"-- {n_circ * n_para * n_obs} tasks required: splitting up into multiple program sets. "
        )
    else:
...
def run_with_program_sets(
    ...
    for n, pset in enumerate(psets):  # potential for catching errors in failed PSETs here
        ...
        pset_results.append(
            device.run(  # noqa: PERF401
                pset, shots=pset.total_executables * pset.shots_per_executable
            ).result()
        )
