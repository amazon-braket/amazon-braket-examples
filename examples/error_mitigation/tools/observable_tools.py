...
PAULI_PRODUCT = {"X": {"Y": "Z", "Z": "Y"}, "Y": {"X": "Z", "Z": "X"}, "Z": {"X": "Y", "Y": "X"}}
...
def matrix_to_pauli(obs: Hermitian | np.ndarray, tol: float = 1e-12) -> list[tuple[complex, str]]:
    ...
    while len(xor_groups) > 0:
        ...
        pauli_coeffs = _coeff_to_pauli(
            _fast_walsh_hadamard(diag), num_qubits=n_qubits, threshold=tol
        )
...
def _pauli_mul(a: str, b: str):
    ...
    return "".join(
        [
            "I" if ai == bi else ai if bi == "I" else bi if ai == "I" else PAULI_PRODUCT[ai][bi]
            for ai, bi in zip(a, b)
        ]
    ), np.prod(
        [
            1 if ai == bi or ai == "I" or bi == "I" else PAULI_PHASE_PRODUCT[ai][bi]
            for ai, bi in zip(a, b)
        ]
    )
...
def _coeff_to_pauli(
    ...
    for n, c in enumerate(entries):
        if abs(c) > threshold:
            paulis.append(
                (c, "".join(["I" if k == "0" else "Z" for k in "{:0{}b}".format(n, num_qubits)]))
            )
    return paulis
...
def pauli_grouping(
    paulis: list[str] | list[tuple[float, str]],
...
def tensor_from_string(pstr: str, include_trivial: bool = False) -> TensorProduct:
