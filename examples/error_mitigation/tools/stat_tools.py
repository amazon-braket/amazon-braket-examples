...
def jackknife_bias_corrected(
    data, func=np.mean, axis=0, jk_estimates: bool = False
...
def perform_regression_ols(
    xs, ys, variances=None, rcond: float = 0.01, error: bool = True
) -> float:
...
def perform_regression(
    ...
    try:
        ...
        coeff, cov = np.polyfit(
            xs, np.log(np.abs(ys)), 1, rcond=rcond, w=1 / np.sqrt(var), cov=True
        )
        ans = np.exp(coeff[1])
        if coeff[1] > coeff_threshold:
            raise ValueError(
                f"exp reg coeff {coeff[1]} is above threshold, performing linear regression"
            )
        if error:
