...
def get_twirled_readout_dist(
    ...
    pset = ProgramSet([processor(v) for v in variants], shots_per_executable=shots // n_twirls)
...
def build_inverse_quasi_distribution(
    reference: dict, second_order: bool = False
...
def process_readout_twirl(counts: dict, index: int | tuple, bit_masks: list | np.ndarray):
...
class SparseReadoutMitigation:
    ...
    def process_single(
        self, result: dict, index: int, pauli_string: str, bit_masks: np.ndarray
    ) -> float:
