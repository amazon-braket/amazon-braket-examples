...
class QCBM:
    ...
    def gradient(self, params: np.ndarray):
        ...
        try:  # try parallel simulations
            result = self.device.run_batch(circuits, shots=self.shots, max_parallel=35).results()
...
# def gradient(qcbm: QCBM, data: np.ndarray, params: np.ndarray):
...
#     """
#     grad = np.zeros(len(params))
#     prob = qcbm.probabilities(params)
#     for i, _ in enumerate(params):
#         params[i] += np.pi / 2.0
#         prob_pos = qcbm.probabilities(params)
#         params[i] -= np.pi
#         prob_neg = qcbm.probabilities(params)
#         params[i] += np.pi / 2.0

#         grad_pos = compute_kernel(prob, prob_pos) - compute_kernel(prob, prob_neg)
#         grad_neg = compute_kernel(data, prob_pos) - compute_kernel(data, prob_neg)
#         grad[i] = grad_pos - grad_neg
#     return grad
