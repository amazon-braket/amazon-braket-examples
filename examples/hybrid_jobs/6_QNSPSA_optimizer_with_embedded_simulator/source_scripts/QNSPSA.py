...
class QNSPSA:
    ...
    Args:
        ...
        blocking (boolean): When set to be True, the optimizer only accept 
            ...
            before update, plus a tolerance. The tolerance is set to be 
            twice of the standard deviation of the loss measurements.
        disable_metric_tensor (boolean): When set to be True, the optimizer is reduced
            to be a (1st-order) SPSA optimizer.
    """
    def __init__(self, lr=1e-3, regularization=1e-3, 
                 finite_diff_step=1e-2, resamplings=1, 
    ...
    def step(self, cost, params):
        ...
        """        
        if self.blocking:
            raise Exception("Use step_and_cost function when blocking is turned on," 
    ...
    def step_and_cost(self, cost, params):
        ...
        """
        params_next = self.__step_core_first_order(cost, params) \
            if self.disable_metric_tensor \
    ...
    def __step_core(self, cost, params):             
        ...
        for i in range(self.resamplings):
            ...
            raw_results = qml.execute(
                grad_tapes + metric_tapes, 
                cost.device, None)
            grad = self.__post_process_grad(
                raw_results[:2], grad_dir)
            metric_tensor = self.__post_process_tensor(
                raw_results[2:], tensor_dirs)
    ...
    def __post_process_grad(self, grad_raw_results, grad_dir):
        ...
        grad = ((loss_forward - loss_backward) /
                (2 * self.finite_diff_step) *
                grad_dir)
        return grad
    
    def __post_process_tensor(self, tensor_raw_results, tensor_dirs):
        tensor_finite_diff = (tensor_raw_results[0][0][0] - 
                              tensor_raw_results[1][0][0] - 
                              tensor_raw_results[2][0][0] + 
                              tensor_raw_results[3][0][0])
        metric_tensor = (-(np.tensordot(tensor_dirs[0], tensor_dirs[1], axes=0) + 
    ...
    def __get_next_params(self, params, gradient):
        ...
        new_params_vec =  np.linalg.solve(
            self.metric_tensor, 
            (-self.lr * grad_vec + 
             np.matmul(self.metric_tensor, params_vec)))
    ...
    def __get_overlap_tape(self, cost, params1, params2):
        ...
        with qml.tape.QuantumTape() as tape:
            ...
            for op in reversed(op_inv):
                op.adjoint()
            #TODO: update this line in PL paper project repo.
    ...
    def __get_tensor_moving_avg(self, metric_tensor):
        ...
        return (self.k / (self.k + 1) * self.metric_tensor + 
