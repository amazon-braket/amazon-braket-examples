...
def pre_run_inject(mock_utils):
    ...
    patch.object(
        AwsDevice, "provider_name", PropertyMock(return_value="rigetti"), create=True
    ).start()
    patch.object(
        AwsDevice, "gate_calibrations", PropertyMock(return_value=None), create=True
    ).start()
    ...
    def mock_run(self, program, *args, **kwargs):
        if isinstance(program, ProgramSet):
            ...
            for _ in range(len(program)):
                ...
                mock_entry.__getitem__.return_value = MagicMock(
                    counts={"00": 50, "11": 50},
                    probabilities={
                        "00": 0.5,
                        "11": 0.5,
