...
def test_readme():
    ...
    for dir_, _, files in os.walk(examples_path):
        for file_name in files:
            if file_name.endswith(".ipynb"):
                is_excluded = [excluded_dir for excluded_dir in EXCLUDED_DIRS if excluded_dir in dir_]
                if not is_excluded:
    ...
    assert extra_in_readme == set(), "There are some (dead) links in the README that do not link to a notebook: "
    assert missing_in_readme == set(), "There are some new notebooks that haven't been added to the README summary: "
